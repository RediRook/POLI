    ...
    (function( $ ) {
        $.widget( "ui.combobox", {
            _create: function() {
                ...
                input = $( "<input id='inputCustomer' name='customer'>" )
                    .appendTo( wrapper )
                    .val( value )

                    .addClass( "customerTest" )
                    .autocomplete({
                        delay: 0,
                        minLength: 0,
...
<div class="row col-sm-12">
    ...
    <p><label class="lblLine" for="datepicker" >Date: </label> <input type="text" id="datepicker">
        <button id="btnSaveSettings" type="button">Go</button>
	</form>
	<div class="row col-sm-12">
	<?php $urlAdd = array ('controller'=>'/Jobs/','action'=>'addJob');
	echo $this->Form->button('Add Job',array('class'=>'btn btn-primary visible-xs','onclick' => "location.href='".$this->Html-> url($urlAdd)."'"));
	?>
	...
	<div class="row col-sm-12">
		...
		<div class="col-sm-3">
			...
			<label for="title">Title: </label><input class="form-control" style="margin-bottom:10px;" type="text" name="title" />

			<?php   echo $this->Form->input('combobox', array('label'=>'Customer:','type'=>'select','div'=>false,'name'=>'customer','options'=>$customers, 'empty'=>'(Choose a customer)'));?>
			<?php   echo $this->Form->input('patcient', array('label'=>false,'type'=>'select','Hidden'=>'Hidden','div'=>false,'name'=>'patient','options'=>$customers));?>
			<br />
			<br />
		...
		<div class="col-sm-3">
			...
			<label>Staff:</label>
				<?php foreach($staffs as $staff){
					echo $this->Form->input($staff['Staff']['staff_name'],array('id' => 'ddstaff','class'=>'StaffCheckbox','type'=>'checkbox','name'=>$staff['Staff']['id']));
				}?>
		</div>
		<div class="col-sm-3">
			<label>Service:</label>
				<?php foreach($services as $service){
				  echo $this->Form->input($service['Service']['name'],array('class'=>'ServiceCheckbox','type'=>'checkbox','name'=>$service['Service']['id']));
				}?>
		</div>
		<div class="col-sm-3">
