...
class PagesController extends AppController {
    ...
    public function display() {
        if($this->Auth->user('id') == null)
        {
            ...
            if($user['User']['type'] == 'Client'){
                $this->redirect(array('controller' => 'Clientcases', 'action' => 'myaccount'));
            }
        }
    ...
    public function home() {
     $this->loadModel('Clientcase');
        ...
        $currentEmp = $this->Employee->find('first', array('conditions' => array('Employee.user_id' => $loggedInUser), 'recursive' => -1));

        $tasks = $this->ResearchTask->find('all', array('conditions' => array('ResearchTask.responsible_id' => $currentEmp['Employee']['id'], 'ResearchTask.status !=' => 'Complete'), 'recursive' => -1));

        $this->set(compact('statuses', 'count', 'documents', 'casenotes', 'docnotes', 'tasks', 'employees'));
    }
